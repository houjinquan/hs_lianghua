...
def save_stock_return(stock_id, path, return_list = [1,2,4,8], Threshold = 0.2, fre = 5, day_num = 242, day_delete_num = 2, skip = 2):
    ###保存本地所有股票的收益率
...
#     y_finally_data_8min = []
    ...
    suspension = data_use_s[:-day_len, 3]
    ...
    while i < int(len(data_use)/(day_len)-fre-1):
        ...
        sub_data_use = data_use[(day_len*i):(day_len*(i+fre)),1:]
        ...
        if len(np.argwhere(res_1  == 0)) < (len(res_1)*Threshold):
            for res in np.arange(len(return_list)):
                return_data = ((res_data[return_list[res]:] - res_data[:-return_list[res]])/res_data[:-return_list[res]]).astype(np.float32)
...
#             res_8 = ((res_data[8:] - res_data[:-8])/res_data[:-8]).astype(np.float32)
            ...
            for j in np.arange(len(sub_time)):
                sec = time.mktime(sub_time[j].timetuple())/(60*60*24) -int(time.mktime(sub_time[j].timetuple())/(60*60*24))
                time_d.append(sec)
...
#     y8_data.extend(data[4])
    ...
    if filelist[:10][-1][:-7] == stock_inf[-1][0][:6]:  
        if(not os.path.exists(save_path + str(len(order)+1))):
            os.makedirs(save_path + str(len(order)+1))
        np.save(save_path + str(len(order)+1) + "/x_data", np.float32(x_data))
...
#         np.save(save_path + str(len(order)+1) + "-y8_data", np.float32(y8_data))
        for data in np.arange(len(return_list)):
            np.save(save_path + str(len(order)+1) + "/y" + str(return_list[data])+ "_data", y_data[data])
...
#     slice_len = 50   ##所有股票的每次切片长度
    ...
    for i in np.arange(len(filelist[:10])):
    ...
    print("计算结束")
#     ceshi = save_stock_return("000004.SZ.npy", path = path)
