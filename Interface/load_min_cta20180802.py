...
#     y_finally_data_8min = []
    ...
    i = 0
    while i < int(len(data_use_s) / day_len - fre - 1):
        sub_data_date = data_use_s[(day_len * (i + fre) - 1), 0].strftime("%Y%m%d")
        sub_data_use = data_use_s[(day_len * i):(day_len * (i + fre) + max(return_list)), 1:]
        sub_time = data_use_s[(day_len * i):(day_len * (i + fre)), 0]
        ...
        if data_use_s[(i + fre - 1) * day_len, 0] in suspension_date:
            i += skip
            continue
