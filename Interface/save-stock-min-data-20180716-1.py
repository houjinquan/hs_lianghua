...
def add_data(data):  
    ...
    if filelist[:30][-1][:-7] == stock_inf[-1][0][:6]:  
        np.save(save_path + stock_inf[-1][0] + "-x_data", x_data)
        np.save(save_path + stock_inf[-1][0] + "-y1_data", y1_data)
        np.save(save_path + stock_inf[-1][0] + "-y2_data", y2_data)
        np.save(save_path + stock_inf[-1][0] + "-y4_data", y4_data)
        np.save(save_path + stock_inf[-1][0] + "-y8_data", y8_data)
        np.save(save_path + stock_inf[-1][0] + "-stock_inf", stock_inf)
