...
def save_stock_return(stock_id, path,Threshold = 0.2, fre = 3, day_num = 242, day_delete_num = 2):
    ###保存本地所有股票的收益率
    ...
    while i < int(len(data_use)/(day_num - day_delete_num)-fre):
        ...
        if len(np.argwhere(res_1  == 0)) < (len(res_1)*Threshold):
            ...
            res_1_r = (res_1 + np.random.uniform(-1,1,len(res_1))/10000).astype(np.float32)
            res_2_r = (res_2 + np.random.uniform(-1,1,len(res_2))/10000).astype(np.float32)
            res_4_r = (res_4 + np.random.uniform(-1,1,len(res_4))/10000).astype(np.float32)
            res_8_r = (res_8 + np.random.uniform(-1,1,len(res_8))/10000).astype(np.float32)
