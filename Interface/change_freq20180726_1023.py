...
def change_freq(data, new_freq):
    if np.ndim(data) == 3:
        ...
        time = temp_data[open_index, 5]

        new_data = np.array([open_price, high_price, low_price, close_price, volume, time])

        return new_data.T.reshape((-1, int(data.shape[1]/new_freq), 6))
