...
def save_stock_return(stock_id, path, return_list = [1,2,4,8], Threshold = 0.2, fre = 5, day_num = 242, day_delete_num = 2, continuous=2):
    # 保存本地所有股票的收益率
    ...
    while i < int(len(data_use) / day_len - fre - 1):
        ...
        if any(logit):
            ...
            skip_num = np.argwhere(breakpoint_position < 0)[0] + 1
            skip = continuous - (fre - skip_num)
